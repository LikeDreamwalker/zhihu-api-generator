...
function getInfo(info, items) {
	for (item in items) {
		...
		// 获取url
		let url = getElementByAttr(
			"meta",
			"itemprop",
			"url",
			items,
			temp
		)[2].getAttribute("content");
		// 获取dateC
		let dateC = getElementByAttr(
			"meta",
			"itemprop",
			"dateCreated",
			items,
		...
		// 获取赞同数
		let upvoteCount = getElementByAttr(
			"meta",
			"itemprop",
			"upvoteCount",
			items,
		...
		// 获取评论数
		let commentCount = getElementByAttr(
			"meta",
			"itemprop",
			"commentCount",
			items,
		...
		let text = getElementByAttr("span", "itemprop", "text", items, temp)[0]
			.textContent;
		// 存储至info对象
		info[item] = new Answers(
			name,
			url,
			dateC,
			dateM,
...
function recursion(mainArray) {
	...
	setTimeout(() => {
		if (status) {
			...
			// 保存为json
			saveJSON(mainArray, authorName + "_answer" + ".json");
			return true;
		}
	}, 0);
}
